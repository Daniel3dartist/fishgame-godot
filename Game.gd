...
#@export var map_scene_path : String = "res://maps/Map1.tscn"
@export var map_scene : PackedScene = load("res://maps/Map1.tscn")
...
signal signal_game_over(peer_id)
...
@rpc("any_peer", "call_local") func _do_game_setup(players: Dictionary) -> void:
	...
	if GameState.online_play:
		var my_id : int = get_tree().get_unique_id()
...
@rpc("any_peer", "call_local") func _do_game_start() -> void:
	if map.has_method('map_start'):
		map.map_start()
	emit_signal("signal_game_started")
...
func _on_player_dead(peer_id) -> void:
	...
	if not game_over and players_alive.size() == 1:
		...
		emit_signal("signal_game_over", player_keys[0])
